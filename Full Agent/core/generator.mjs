...
Analysis of exposed operational directory telemetry at \`${defang(intel.host)}:${intel.port || "443"}\` identified active staging artifacts and offensive tooling.

### 1. Observed Tooling & Capabilities
${intel.tools.map((t) => `* **${t}**`).join("\n")}

${intel.deobfuscation && intel.deobfuscation.recoveredKeys.length > 0 ? `### 2. Deobfuscated Payloads & Encryption Keys\n\n${intel.deobfuscation.recoveredKeys.map((k) => `* **Recovered Key**: \`${k.ascii}\` (Hex: \`${k.hex}\`) from ${k.source}`).join("\n")}\n` : ""}

${intel.aiAnalysis && intel.aiAnalysis.used ? `### 3. Local AI Technical Breakdown (${intel.aiAnalysis.model})\n\n${intel.aiAnalysis.analysisText}\n` : ""}

### 4. Verified High-Fidelity Indicators (IOCs)
