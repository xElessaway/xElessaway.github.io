 ...
 */
export async function generateContent(intel, config) {
  ...
  // 1. Generate Threat Dossier (Strictly High-Confidence Verified IOCs)
...
`;
  ...
  // 2. Generate Blog Report
  const blogMarkdown = `---
title: "Technical Teardown of Staging Infrastructure at ${defang(intel.host)}"
description: "Forensic analysis of exposed malware staging and C2 infrastructure identified at ${defang(intel.host)}."
...
Analysis of exposed operational directory telemetry at \`${defang(intel.host)}:${intel.port}\` identified active staging artifacts and offensive tooling.
...
`;
  ...
  console.log(`\n📄 [Generated Dossier] ${actorFile}`);
  console.log(`📄 [Generated Report] ${blogFile}`);

  return { actorSlug, reportSlug, actorFile, blogFile };
}
