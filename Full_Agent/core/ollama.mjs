...
// 3. Interactive Multi-Turn Chat & Conversational Refinement
export async function chatWithOllama(messages, sessionContext = {}, modelName = null) {
  const status = await checkOllamaStatus();
  if (!status.online) {
    return {
      message: "⚠️ Ollama is currently offline. Please ensure Ollama is running (`ollama serve`) to enable AI chat reasoning."
    };
  }

  const model = modelName || status.selectedModel || "qwen2.5:3b";

  const systemContext = `You are the AI Threat Intelligence & DFIR Copilot inside the Inked Intel Studio.
You are assisting the user (Ahmed Elessaway) in analyzing target infrastructure, investigating malware payloads, and refining Threat Dossiers & DFIR Reports.

Current Target Telemetry Context:
- Target Host: ${sessionContext.host || "None specified"}
- Port: ${sessionContext.port || "N/A"}
- Server: ${sessionContext.serverHeader || "N/A"}
- Identified Tools: ${(sessionContext.tools || []).join(", ") || "None"}
- MITRE TTPs: ${(sessionContext.ttps || []).join(", ") || "None"}
- Discovered Secrets: ${JSON.stringify(sessionContext.secrets || [])}
- Carved Binaries: ${JSON.stringify((sessionContext.deobfuscation?.carvedBinaries || []).map((b) => ({ name: b.filename, type: b.type, sha256: b.sha256 })))}

Instructions:
1. Provide concise, expert DFIR & threat intelligence responses.
2. If the user asks to edit or rewrite the Dossier or Report, provide the updated text and explain the rationale.
3. If the user asks about malware mechanics, explain the deobfuscation layers, encryption keys, and execution flows accurately.
4. Keep tone professional, analytical, and direct.`;

  // Format chat into prompt
  ...
  try {
    ...
    if (res.ok) {
      ...
      return { message: `Ollama error: HTTP ${res.status}` };
    }
  } catch (err) {
    return { message: `Ollama error: ${err.message}` };
  }
}
