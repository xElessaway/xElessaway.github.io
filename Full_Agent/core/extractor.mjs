...
export async function extractIntelligence(triageResult, modelName = null) {
  ...
  // 1. Run Automated Recursive Deobfuscation & In-Memory PE Carving
  const deobfResults = await analyzeAndDeobfuscateFiles(files);
  ...
  // Carved Embedded PE/DLL Binaries
  if (deobfResults.carvedBinaries && deobfResults.carvedBinaries.length > 0) {
    deobfResults.carvedBinaries.forEach((bin) => {
      highConfidenceIOCs.push({
        type: "Carved In-Memory Binary SHA-256",
        value: bin.sha256,
        role: `${bin.type} (${bin.filename}) extracted from ${bin.source}`,
        confidence: "High (100%)"
      });
    });
  }
